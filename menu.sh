...
export COLOR1="$(cat /etc/maxisovpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/maxisovpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
echo "IP=$host" > /var/lib/maxisovpn-pro/ipvps.conf
echo ""
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/Mjoyvpn/MAXIS/main/install_up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/Mjoyvpn/MAXIS/main/version)
...
serverV=$( curl -sS https://raw.githubusercontent.com/Mjoyvpn/MAXIS/main/version)
if [ "$Isadmin" = "ON" ]; then
